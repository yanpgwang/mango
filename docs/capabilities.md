...
| [First-party SDKs](sdk.md) | Preview | Go, Python (sync/async), and TypeScript/JavaScript source packages cover the current OpenAPI operations with generated types, pagination, multipart and streaming transports. Packages are not yet published as supported registry releases; HTTP conformance uses test-only storage/model implementations. Server capability limits still apply. |
