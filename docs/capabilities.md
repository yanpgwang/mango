...
| Sandbox tools | Limited | `bash`, `read`, `write`, `edit`, `glob`, and `grep`, plus provider-native Web Search/Fetch. `read` supports 1-based inclusive line ranges and is capped at 64 KiB inside the sandbox; larger files and persisted tool outputs use bounded `bash` byte slices. Docker is the default; host-process execution is not selectable. Docker and the Preview remote providers expose separately admitted resource capabilities. |
