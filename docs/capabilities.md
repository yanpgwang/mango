...
| Sandbox tools | Limited | `bash`, `read`, `write`, `edit`, `glob`, and `grep`, plus provider-native Web Search/Fetch. `read` supports 1-based inclusive line ranges and is capped at 64 KiB inside the sandbox; larger files and persisted tool outputs use bounded `bash` byte slices. OpenSandbox is the only backend. Local/CI uses its Docker runtime; there is no host-process or direct-Docker fallback. |
...
| [Files](api/files.md) | Limited | Configured S3-compatible storage, crash-recoverable intents, reusable snapshotted UTF-8 outcome rubrics, bounded UTF-8 File documents snapshotted into `user.message`, downloadable Session Resource copies, and OpenSandbox publication of regular files beneath `/mnt/session/outputs` before idle. Client uploads are intentionally not downloadable. File-sourced images/PDFs and distributed reconciliation remain open. |
| [Session Resources](api/session-resources.md) | Limited | Independent File copies, create-time Memory attachments, and create-time public HTTPS Git repository snapshots frozen to an exact commit. Runtime File attach/detach works. OpenSandbox Git worktrees are writable and restore offline from Mango storage. Private repository credentials, recursive submodules, LFS objects, repository Skill discovery, and runtime Git attach/detach remain open. |
| [Skills](api/skills.md) | Limited | Custom Skill lifecycle, immutable Version pins, strict bundle validation, OpenSandbox materialization, Agent-scoped paths, and on-demand instruction injection. Skill copies are permission-hardened and the main instruction entrypoint is reconciled, but they are not native read-only mounts. Self-hosted Sessions reject effective primary or roster Skills at creation. External catalogs, repository sources, and Environment Worker activation are not implemented. |
| [Memory](api/memory.md) | Limited | Store/Memory/Version lifecycle, immutable history, SHA-256 preconditions, OpenSandbox-managed read/write or read-only volume mounts, and deletion-time writeback. Automatic retention is not implemented. |
...
| [Sandbox runtime](sandboxes.md) | Limited / Preview | OpenSandbox is the sole backend and has durable bindings, Files, Git, Outputs, Skills, Memory, and limited egress. Local/CI uses its pinned Docker runtime with only the OpenSandbox service holding the socket. Kubernetes with a required Kata RuntimeClass is the production candidate and remains Preview until the pinned stack passes repeatable operational and security gates. |
| Distributed operation | Limited | API and worker roles scale independently around PostgreSQL, Temporal, and NATS. Worker Versioning, shared OpenSandbox control-plane operation, distributed Files reconciliation, and production rollout evidence remain open. |
...
- The local OpenSandbox Docker runtime shares the host kernel and is not a
  hardened hostile multi-tenant boundary. The OpenSandbox service, not Mango's
  worker, is trusted with daemon access.
...
PostgreSQL/Temporal/NATS/MinIO/OpenSandbox-on-Docker service tests.
