...
| [First-party SDKs](sdk.md) | Preview | Go, Python (sync/async), and TypeScript/JavaScript clients cover the current OpenAPI operations with generated types, pagination, multipart and streaming transports. The Go SDK also provides provider-neutral Work polling and a single-Session tool runner with stream/history recovery, confirmation gates, ambiguous-write reconciliation, and lease-loss fencing. TypeScript `mango-sdk@0.1.0-alpha.1` is published on npm; Python `mango-sdk==0.1.0a1` is published on PyPI. Alpha publication does not establish a stable contract. HTTP conformance uses test-only storage/model implementations. Server capability limits still apply. |
...
| [Environment Work](api/environment-work.md) | Limited | Self-hosted worker leases, polling, state transitions, reclaim, and Session activation are implemented. Poll returns the CMA-shaped base64url Work secret payload; after Ack its per-claim `sessions_token` authorizes only that Work's Heartbeat/Stop, the claimed Session's read/stream/tool-result routes, and pinned File/Skill inputs. TTL is capped at five minutes; expiry, Stop, and reclaim fence writes and close old streams. Reclaim rotates the token and only its digest is stored. The Go `WorkPoller` carries the payload across the redacted Ack response, and `SessionToolRunner` executes one claimed Session without choosing a sandbox. A composed Environment worker, Environment-scoped polling credentials, and health-check Work are not implemented; supervisors currently use a Workspace key. |
